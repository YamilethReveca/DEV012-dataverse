...
export const computeStats = (data, decimalPlaces = 2) => {
  const totalWeight = (data.reduce((sum, pokemon) => sum +(pokemon.fact.weight), 0)).toFixed(decimalPlaces); 
  const averageWeight = (totalWeight / data.length).toFixed(decimalPlaces); 
  const maxWeight = Math.max(...data.map(pokemon => (pokemon.fact.weight))).toFixed(decimalPlaces); 
  const minWeight = Math.min(...data.map(pokemon => (pokemon.fact.weight))).toFixed(decimalPlaces); 
